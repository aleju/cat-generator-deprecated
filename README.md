...
*Nearest neighbours in the training set of 16 generated 16x16 images (distance measured by 2-Norm / `torch.dist()`). The 16 selected images were the "best" ones among 1024 images according to the rating by D, hence some similarity with the training set is expected.*
...
* If you want to sample from G2 or G3 (`train2.lua` / `train3.lua`) you will have to edit `sample.lua` and change the line `local G = file.G` (in method `loadModels()`) to `local G = file.G2` (G2) or `local G = file.G3` (G3).

Notes:
* Results of experiments seem to not be reproduceable (when rerunning with the same hyperparameters). (I don't know the reason for that, as I set all seeds (`math.randomseed()`, `torch.manualSeed()`, `cutorch.manualSeed()`).) Because of that, you may have to run experiments several times to get results of the same quality as shown above (I found that to be extremely entertaining -___-). You should however get on (basically) every run images that clearly resemble cats.
...
Choosing other sizes for the hidden layer also seemd to worsen results.
...
G3 is similar to G/G1. It takes in the image generated by G2 as a vector and returns a similarly sized image with one hidden layer of 1024 nodes in between, i.e. the architecture is:
...
Adding Batch Normalization did not work - it lead to D having basically immediately 100% accuracy, leaving G no time to learn anything (or maybe it somehow messed up the gradients for G).
...
Similarly in the case of `train3.lua` another G (G3), which just has a normal 1024-neuron hidden layer, is added at the end.
...
images generated by the chain of lower Gs (e.g. 16x16 G then 16to22 G) are also fed into the coarse to fine G, just like the real images from the training set.
