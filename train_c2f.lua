...
while true do
    ...
    if not OPT.noplot then
        ...
        disp.image(to_plot_val, {win=OPT.window, width=2*10*IMG_DIMENSIONS[3], title=string.format("[VAL] Coarse, GT, G img, GT diff, G diff (%s epoch %d)", OPT.save, EPOCH)})
        disp.image(to_plot_train, {win=OPT.window+1, width=2*10*IMG_DIMENSIONS[3], title=string.format("[TRAIN] Coarse, GT, G img, GT diff, G diff (%s epoch %d)", OPT.save, EPOCH)})
